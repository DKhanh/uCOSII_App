...
/* @May 28th, 2010 By: Amr Ali Abdel-Naby */
/* Added definitions for interrupts */
...
/* @May 28th, 2010 By: Amr Ali Abdel-Naby */
/* Added the following 2 APIs for interrupts */
DWORD WINAPI OSInterruptW32( LPVOID lpParameter );
...
HANDLE              InterruptEvent[NINTERRUPTS];        /* Interrupt handling thread variables                                                      */
HANDLE              OSInterruptW32Handle;
void                (*interruptTable[NINTERRUPTS])();
INT32U              OSInterruptW32Id = 0;
/*
*********************************************************************************************************
...
void OSStartHighRdy()
{
    DWORD  dwID;
    /* @May 28th, 2010 By: Amr Ali Abdel-Naby */
    /* Added interrupt threads and events creation */
    int i;
    char temp[256];
    ...
    {   
        ...
        InterruptEvent[i] = CreateEvent(NULL, TRUE, FALSE, temp);
        interruptTable[i]  = OSDummyISR;
    }
    OSInterruptW32Handle= CreateThread(NULL, 0, OSInterruptW32, NULL, 0, &OSInterruptW32Id);
    SetPriorityClass( OSInterruptW32Handle, THREAD_PRIORITY_HIGHEST);

#ifdef SET_AFFINITY_MASK
    if( SetThreadAffinityMask( OSInterruptW32Handle, 1 ) == 0 ) {
#ifdef OS_CPU_TRACE
        OS_Printf("Error: SetThreadAffinityMask\n");
#endif
    }
...
DWORD WINAPI OSTickW32( LPVOID lpParameter )
{
    ...
    while(!OSTerminateTickW32)
    {
        OSTickISR();
#ifdef WIN_MM_TICK
        if( WaitForSingleObject(OSTickEventHandle, INFINITE) == WAIT_TIMEOUT)
        {
            #ifdef OS_CPU_TRACE
...
DWORD WINAPI OSInterruptW32( LPVOID lpParameter )
{
    ...
    while (1)
    {
        eventType = WaitForMultipleObjects(NINTERRUPTS, InterruptEvent, FALSE, INFINITE);
        if (eventType == WAIT_TIMEOUT)
        {
            #ifdef OS_CPU_TRACE
                OS_Printf("Error: Interrupt Timeout!\n");
                continue;
            #endif
        }
        ...
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 0 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[0]);
            interruptTable[0]();                                                        
        } else if (eventType==WAIT_OBJECT_0+1)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 1 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[1]);
            interruptTable[1]();
        } else if (eventType==WAIT_OBJECT_0+2)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 2 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[2]);
            interruptTable[2]();
        } else if (eventType==WAIT_OBJECT_0+3)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 3 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[3]);
            interruptTable[3]();
        } else if (eventType==WAIT_OBJECT_0+4)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 4 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[4]);
            interruptTable[4]();
        } else if (eventType==WAIT_OBJECT_0+5)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 5 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[5]);
            interruptTable[5]();
        } else if (eventType==WAIT_OBJECT_0+6)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 6 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[6]);
            interruptTable[6]();
        } else if (eventType==WAIT_OBJECT_0+7)
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Debug: Interrupt 7 Processed!\n");
            #endif
            ResetEvent(InterruptEvent[7]);
            interruptTable[7]();
        } else
        {   
            #ifdef OS_CPU_TRACE
                OS_Printf("Error: Interrupt Received Outside of Range!\n");
            #endif
...
/*
*********************************************************************************************************
*                                          Dummy ISR -  OSDummyISR()
*
* Description: This function is called for an interrupt that does not have ISR registered
*********************************************************************************************************
*/
void OSDummyISR(void)
{
    #ifdef OS_CPU_TRACE
        OS_Printf("Error: There is an interrupt without ISR installed!\n");
    #endif
}
/*
*********************************************************************************************************
